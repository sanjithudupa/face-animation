...
for tri in tri1:
    ...
    _, mask_triangles_designed = cv2.threshold(area_gray, 1, 255, cv2.THRESH_BINARY_INV)
    warped = cv2.bitwise_and(warped, warped, mask=mask_triangles_designed)

    area = cv2.add(area, warped)

    overlay(outputImage[y1: y1 + h1, x1: x1 + w1], area)
...
# final = cv2.rectangle(final, (x, y), (x + h, y + h), (255, 0, 0), 3)

plt.imshow(outputImage)
cv2.imwrite('og.png', masked)
cv2.imwrite('warped.png', warped)

cv2.imwrite('out.png', cv2.cvtColor(outputImage, cv2.COLOR_RGB2BGRA))
